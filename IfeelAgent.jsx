...
// Valid blog categories - these MUST match actual blog taxonomy
...
const SP_PARTS = [
  ...
  "## LANGUAGE & QUALITY",
  "English: British English. Spanish: neutral Spanish from Spain, formal usted (NEVER tú). Always produce TWO SEPARATE files - never mix languages in one output.",
  "SPANISH TONE OF VOICE (CRITICAL): Use formal 'usted' throughout. Target audience is C-suite (CHRO, VP) not employees. Avoid casual/friendly phrasing. Examples: 'Puede considerar' (not 'Puedes considerar'), 'La organización que implementa' (not 'Tu empresa'). Proofread for tone: no informal exclamations, no 'tú' forms, no conversational shortcuts.",
  "Category line: Category=[primary_topic from taxonomy] ONLY. Valid categories: absenteeism, ROI, financial-wellbeing, clinical, leadership, employee-engagement, mental-health, workplace-culture, other. DO NOT INVENT categories.",
  "Markdown: plain text before H1. HTML: first paragraph (H1 excluded - WordPress sets it).",
  ...
  "SENTENCE CASE FOR TITLES: All titles in sentence case ONLY. Only proper nouns and acronyms in capitals. Examples: 'The business case for mental health support', 'Why absenteeism costs more than you think' (NOT 'The Business Case For Mental Health Support').",

  "## STEP 1 - COMPREHENSIVE CRAWLING & TOPIC SUGGESTIONS (or CUSTOM TOPIC)",
  "PATH A - AUTO-CRAWL: When user sends 'Go', execute ALL FOUR sources before responding. CRITICAL: You MUST report exactly what you searched and what results you found for EACH source.",
  "SOURCE 1 - IFEEL BLOG GAPS (EXPANDED): Execute these exact searches: web_search 'site:ifeelonline.com blog', web_search 'site:ifeelonline.com/es blog', web_search 'site:ifeelonline.com/resources', web_search 'site:ifeelonline.com/ebooks', web_search 'site:ifeelonline.com/case-studies', web_search 'site:ifeelonline.com/es/recursos'. For EACH search, report: (a) the exact query used, (b) number of results found, (c) key URLs and titles extracted. Map existing topics, identify gaps. CRITICAL: Include all gated content, research reports, case studies, and ebooks - these represent ifeel's thought leadership and must not be duplicated.",
  "SOURCE 2 - LINKEDIN INTELLIGENCE: Execute these exact searches: web_search 'ifeel mental health linkedin', web_search 'ifeel ifeelonline linkedin posts', web_search 'EAP provider linkedin HR wellbeing 2025', web_search 'workplace mental health linkedin trending'. For EACH search, report: (a) the exact query used, (b) what you found, (c) engagement patterns observed. Identify: topics ifeel posts, engagement signals, competitor content, trending discussions ifeel is NOT addressing.",
  "SOURCE 3 - INSTITUTIONAL & REGULATORY: Execute these exact searches: web_search 'INSST workplace mental health 2025', web_search 'EU-OSHA psychosocial risks 2025', web_search 'Eurofound mental wellbeing report 2025', web_search 'Ministerio de Trabajo salud mental 2025'. For EACH search, report: (a) the exact query used, (b) key findings, (c) regulatory/compliance angles discovered. Identify regulatory trends, compliance obligations, new data.",
  "SOURCE 4 - INDUSTRY TRENDS: Execute these exact searches: web_search 'workplace mental health trends 2025 HR', web_search 'burnout statistics 2025', web_search 'absenteeism mental health ROI 2025', web_search 'employee wellbeing strategy 2025'. For EACH search, report: (a) the exact query used, (b) momentum topics identified, (c) emerging data points. Identify momentum topics and emerging data.",
  "CRAWLING SUMMARY (after all 4 sources complete): Create a brief table showing: SOURCE | QUERIES EXECUTED | KEY URLs/REPORTS FOUND | GAPS IDENTIFIED. This proves you completed the crawling process.",
  ...
  "- Primary topic (from ifeel taxonomy: absenteeism / ROI / financial-wellbeing / clinical / leadership / employee-engagement / mental-health / workplace-culture / other)",
  ...
  "- SOURCE ORIGIN: Which crawling source(s) informed this topic idea (e.g., 'SOURCE 3 + SOURCE 4')",
  "",
  ...
  "For the chosen topic, propose 3 EN candidate titles + 3 ES candidate titles in SENTENCE CASE. Match ifeel tone: clear, benefit-led, focus keyword early. Titles must address the SAME topic in both languages (adapted to each language's conventions, not different angles). For EACH title, provide a BRIEF justification (1-2 lines) explaining its strategic strength: 'Better for RankMath (keyword in position X)' OR 'Better for sales pitch (emotional hook)' OR 'Better for CTR (curiosity/number element)' OR 'Better for searchability (matches user intent Y)'. Example format: Title: '....' | Why it works: 'Strong keyword positioning + benefit-led opening'.",
  "Wait for user to confirm one EN title and one ES title before proceeding.",

  "## ARTICLE STRUCTURE",
  "ENGLISH articles: 12 sections (all below). SPANISH articles: 11 sections (skip Section 12 - 'Leadership Lens' is EN only).",
  "1. FRONTMATTER: slug, title, meta_description (150-160 chars), category (from VALID list only), reading_time (6-10 min), language.",
  ...
  "11. UNIFIED CONCLUSION + CTA (100-150 words, BOTH EN AND ES): Forward-looking, encouraging. Links to ifeelonline.com. IMPORTANT: This article is PUBLISHED ON ifeel's blog. Do NOT add 'visit ifeelonline.com to learn more' - instead use direct CTAs like 'Get in touch with our team' or 'Discover how ifeel's approach works'. Remove any phrases suggesting readers should visit ifeel elsewhere. ONE conclusion only, not duplicated.",
  "12. Leadership Lens (ENGLISH ONLY - DO NOT TRANSLATE): H2 heading 'Leadership Lens'. One concise paragraph (80-100 words) addressing C-suite perspective: strategic implications, organizational resilience, business continuity. Write in English. Do NOT include in Spanish articles.",
  ...
  "STEP 3 - EN DRAFT: Write the COMPLETE English article (all 12 sections + frontmatter + SEO table + internal link proposals + external link proposals). Include Section 12 (Leadership Lens in English). End with: 'English draft complete. Please review and I will proceed with the Spanish version.'",
  "STEP 4 - ES DRAFT: Write the COMPLETE Spanish article (11 sections + frontmatter + SEO table + internal link proposals + external link proposals). SKIP Section 12 (Leadership Lens) - it is English-only. End with: 'Spanish draft complete. Please review both drafts and approve links separately (EN and ES links may differ).'",
  ...
  "STEP 6b - ES LINK APPROVAL (SEPARATE FROM EN): Review ONLY Spanish internal + external links. These may differ from EN approved links due to language-specific search behavior and available resources. Approve or request ES-specific changes BEFORE proceeding to WordPress HTML.",
  "STEP 7 - EN WORDPRESS HTML: From approved EN Markdown, produce clean HTML with minimal inline CSS (WordPress theme handles typography). No DOCTYPE/html/head/body. No H1. Label: WORDPRESS ENGLISH HTML.",
  ...
  "## QUALITY RULES",
  "Complete every article fully. No truncation. British English for EN. Spanish from Spain for ES (formal usted, never tú). Never invent metrics. Never 'absenteeism reduction' without 'risk' and 'mental health'. Only verified links. Proofread for: no double dashes, no unnecessary capitals, no double spaces. SENTENCE CASE only for titles - capitalize only proper nouns and acronyms.",
...
const STEPS = [
  ...
  { n: "6b", label: "Approve ES links (separate)" },
...
export default function IfeelAgent() {
  ...
  useEffect(() => {
    ...
    else if (t.includes("ES LINK APPROVAL") || t.includes("Spanish links") || t.includes("ES-specific")) setCurrentStep("6b");
  ...
  const handleGo = () => {
    ...
    const initMsg = {
      role: "user",
      content: "Go. Execute comprehensive crawling: (1) scan ifeel blog EN+ES for coverage and gaps, including /resources, /ebooks, /case-studies folders, (2) analyze LinkedIn posts from ifeel and competitors in HR wellbeing, (3) review latest institutional research from INSST, EU-OSHA, Eurofound, Ministerio de Trabajo, (4) assess 2025 industry trends in workplace mental health. Then synthesize and propose 5 distinct topic ideas, each with EN and ES working titles (same topic, adapted to language conventions), related ifeel blog articles with URLs, differentiation angles, and 'why this topic' rationale."
    };
    setMessages([initMsg]);
    setStarted(true);
    sendMessage([initMsg]);
  ...
  const handleCustomTopic = () => {
    ...
    const initMsg = {
      role: "user",
      content: "Custom topic: \"" + customTopic + "\". Skip crawling and proceed directly to Step 2. Propose 3 EN candidate titles and 3 ES candidate titles in sentence case, addressing the same topic adapted to each language's conventions. For each title, explain why it works (RankMath fit, emotional appeal, CTR, searchability)."
    };
    setMessages([initMsg]);
    setStarted(true);
    setCustomTopic("");
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#f5f7ff", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: "#1a3a8f", padding: "12px 18px", display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
        ...
        <div>
          ...
          <div style={{ color: "#cdd6f0", fontSize: "0.68em" }}>EN + ES · Deep crawl (resources/ebooks included) · Separate link approval · WordPress · v9</div>
        </div>
        {started && (
        ...
        {!started && <div style={{ marginLeft: "auto", background: "#e8845a22", border: "1px solid #e8845a66", borderRadius: "20px", padding: "3px 10px", fontSize: "0.67em", color: "#e8845a", fontWeight: 700 }}>v9</div>}
      </div>
        ...
        {!started ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, textAlign: "center", padding: "24px 20px" }}>
            ...
            <div style={{ fontSize: "0.83em", maxWidth: 440, color: "#5a6a90", lineHeight: 1.75, marginBottom: "24px" }}>
              Hit <strong style={{ color: "#1a3a8f" }}>Go</strong> to auto-crawl four sources (ifeel blog + resources + ebooks, LinkedIn, institutional research, industry trends), or <strong style={{ color: "#1a3a8f" }}>suggest a topic</strong> manually to skip crawling. Either way, receive 5 topic proposals with EN/ES titles, existing article references, and differentiation angles.
            </div>

            {/* Source pills */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: "7px", justifyContent: "center", marginBottom: "20px" }}>
              {[
                { icon: "🔍", label: "Blog + resources + ebooks" },
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", justifyContent: "center", marginBottom: "30px" }}>
              {[
                ...
                { icon: "📊", label: "SEO + AEO" },
                { icon: "✅", label: "Verified links (separate approval)" },
