...
app.post("/make-server-9633489f/validate-token", async (c) => {
  ...
  try {
    ...
    try {
      ...
      return c.json({ success: false, valid: false, error: "Token inválido o expirado" }, 401);
    }
...
app.post("/make-server-9633489f/submit-demo-request", async (c) => {
  ...
  try {
    ...
    try {
      ...
      console.error('❌ Token inválido:', jwtError.message);
      return c.json({ success: false, error: "Token inválido o expirado", redirect: null }, 401);
    }
    ...
    if (!hubspotApiKey) {
      ...
      return c.json({ success: true, message: "Token válido pero HubSpot no configurado", redirect: '/gracias', skipped: true });
    }
    ...
    try {
      ...
      if (searchResult.results && searchResult.results.length > 0) {
        ...
        await sendConfirmationEmail(contactData);
        return c.json({ success: true, message: "Contacto ya existe en HubSpot", redirect: '/gracias', contactExists: true, contactId: contactId });
    ...
    if (!contactExists) {
      ...
      return c.json({ success: true, message: "Nuevo contacto creado en HubSpot", redirect: '/gracias', contactExists: false, submitted: true });
    }

    console.error('❌ Flujo inesperado: no se manejó correctamente el caso');
    return c.json({ success: false, error: "Flujo inesperado", redirect: null }, 500);

  } catch (error) {
    console.error('❌ Error en flujo de demo:', error);
    return c.json({ error: `Error: ${error.message}`, redirect: null }, 500);
  }
});
...
app.get("/make-server-9633489f/register-interest", async (c) => {
  ...
  let downloadId;
  try {
    const decoded = await jose.jwtVerify(token, JWT_SECRET_UINT8);
...
app.get("/make-server-9633489f/download-logs", async (c) => {
  try {
    ...
    return c.json({ success: true, downloads: data || [], count: data?.length || 0 });
  } catch (error) {
    return c.json({ error: error.message }, 500);
  }
});
...
app.get("/make-server-9633489f/test-sendgrid", async (c) => {
  ...
  if (!sendgridApiKey) {
    return c.json({ error: 'SENDGRID no configurada', diagnostics }, 500);
  }
  ...
  try {
    ...
    return c.json({
      success: true,
      diagnostics,
      apiTest: { status: testResponse.status, ok: testResponse.ok, result: testData },
      emailTest: { status: emailStatus, ok: emailOk, error: emailError, recipient: testEmail }
    });
  } catch (error) {
    return c.json({ error: 'Error al probar SendGrid', diagnostics, details: error.message }, 500);
  }
});
...
app.get("/make-server-9633489f/test-hubspot-form", async (c) => {
  ...
  try {
    ...
    return c.json({ success: false, error: `Error: ${error.message}`, portalId: HUBSPOT_PORTAL_ID, formGuid: HUBSPOT_FORM_GUID }, 500);
  }
});
